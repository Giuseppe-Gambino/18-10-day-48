...
export class TodosService {
  ...
  getTasksWithUser(users: iUsers[]): iTodos[] {
    return this.todos.map((task) => {
      ...
      return {
        ...
        userImg: foundUser.image,
      };
    });
  }

  getByNameService(tasksArr: iTodos[], nameForm: string): iTodos[] {
    if (nameForm) {
      return tasksArr.filter((task) => task.userName?.includes(nameForm));
    } else {
      return tasksArr;
    }
  }
