...
export class UsersService {
  ...
  getUserWithTasks(todos: iTodos[]): iUsers[] {
    this.user.map((utente) => {
      const taskTrovata = todos.filter((todos) => todos.userId === utente.id);
      utente.task = taskTrovata;
    });
    return this.user;
  }

  getByNameService(tasksArr: iUsers[], nameForm: string): iUsers[] {
    if (nameForm) {
      return (tasksArr = tasksArr.filter((users) =>
        users.firstName.includes(nameForm)
      ));
    } else {
      return tasksArr;
    }
